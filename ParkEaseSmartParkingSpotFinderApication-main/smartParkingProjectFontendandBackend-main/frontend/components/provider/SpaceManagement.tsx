...
export default function SpaceManagement() {
    ...
    const toggleStatus = async (id: string) => {
        try {
            ...
            await fetch(
                `${BASE_URL}/api/provider/slots/${id}/toggle`,
                {
                    method: 'PATCH',
                    headers: {
                        Authorization: `Bearer ${token}`,
                ...
                { text: "Cancel", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            await fetch(
                                `${BASE_URL}/api/provider/slots/${id}`,
                                {
                                    method: 'DELETE',
                                    headers: {
                                        Authorization: `Bearer ${token}`,
    ...
    const renderSlotItem = ({ item, index }: { item: Slot; index: number }) => (
        ...
        >
            <View className="flex-row items-center">
                <View
                    className={`w-12 h-12 rounded-2xl justify-center items-center ${item.status === 'active'
                            ? 'bg-indigo-50'
                            : 'bg-gray-100'
                        }`}
                >
            ...
            <View className="flex-row items-center gap-3">
                <TouchableOpacity
                    ...
                    className={`px-4 py-2 rounded-xl ${item.status === 'active'
                            ? 'bg-green-50'
                            : 'bg-red-50'
                        }`}
                >
                    <Text
                        className={`font-bold text-[10px] uppercase tracking-widest ${item.status === 'active'
                                ? 'text-green-600'
                                : 'text-red-600'
                            }`}
                    >
                        ...
                        <View className="flex-row gap-4 mb-10">
                            {(['car', 'bike', 'ev'] as const).map(type => (
                                <TouchableOpacity
                                    ...
                                    className={`flex-1 p-4 rounded-2xl border-2 items-center ${newSlot.type === type
                                ...
                                >
                                    ...
                                    />
                                    <Text
                                        className={`font-bold mt-2 capitalize ${newSlot.type === type
                                                ? 'text-indigo-600'
                                                : 'text-gray-400'
                                            }`}
                                    >
